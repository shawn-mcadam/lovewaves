...
\begin{figure}[htbp]
	...
	\label{fig:setup}
\begin{tikzpicture}
\draw[help lines, color=gray!30, dashed] (-3.9,-1.9) grid (4.9,2.9);
\draw[->,ultra thick] (-5.75,3.5)--(-4.5,3.5) node[right]{$+x$};
\draw[->,ultra thick] (-5.5,3.75)--(-5.5,2.5) node[below]{$+z$};
\draw[-, ultra thick] (-5,3)--(5,3);
\draw[-, ultra thick] (-5,1)--(5,1);
\node (n1) at (0,2) {Medium $1$};
\node (n2) at (0,0) {Medium $2$};
\node (n3) at (4,3.25) {$z=0$};
\node (n3) at (4,1.25) {$z=L$};
\node (n3) at (0,3.5) {$\partial u/\partial z|_{z=0} = 0$};
\node (n3) at (0,-1.5)  {$u(x,\infty,t)=0$};
...
%\]
%$L\in(0,H)$, and $\Omega_1,\Omega_2\in[0,k]$ are chosen such that $u$ and $u_z$ are continuous.
...
%approximately constant velocity equal to $v$. The solution does not evolve beyond this state. 
...
%\centering
%
%The parameters are $c_1 = 0.5$, $c_2 = 1$, $v=0.56115873910026276$, $W=2\pi$, $L=8$, $A=H=1$, $k=2$, and $h = 0.02$.
%\end{figure}
%
%\begin{figure}[htbp]
...
%The later shock waves result in rippling patterns.
...
\begin{figure}[htbp]
	...
	\includegraphics[width=0.7\linewidth]{figures/gaussian_below_c1c2/overhead}
\footnotesize
\centering
...
\begin{figure}[htbp]
	...
	\includegraphics[width=0.7\linewidth]{figures/gaussian_above_c1c2/overhead}
\footnotesize
\centering
...
\begin{figure}[htbp]
	...
	\includegraphics[width=0.7\linewidth]{figures/gaussian_below_c2c1/overhead}
\footnotesize
\centering
